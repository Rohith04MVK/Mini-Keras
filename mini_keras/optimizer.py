...
class Adam(BaseOptimizer):
    ...
    def update(self, learning_rate, w_grads, b_grads, step):
        ...
        for layer in self.trainable_layers:
            ...
            dw = (
                learning_rate
                * v_corrected[layer_dw]  # noqa: W503
                / (np.sqrt(s_corrected[layer_dw]) + self.epsilon)  # noqa: W503
            )
            db = (
                learning_rate
                * v_corrected[layer_db]  # noqa: W503
                / (np.sqrt(s_corrected[layer_db]) + self.epsilon)  # noqa: W503
            )
